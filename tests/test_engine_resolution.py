...
Asserts the engine wires the M1-03 resolver into the deterministic core: a hot CPI episode's
conditional cells become concrete directions, a cool episode's the inverses, an in-line episode's
...
def _replay_by_symbol(name: str, con: duckdb.DuckDBPyConnection) -> dict[str, str]:
    episode = Episode.model_validate_json((EPISODES_DIR / name).read_text())
    return {w.instrument: str(w.direction) for w in replay(episode, con=con)}


def test_hot_cpi_resolves_conditional_cells(seeded_con: duckdb.DuckDBPyConnection) -> None:
    # given the hot CPI episode over the five core conditional cells
    # when it is replayed
...
def test_in_line_cpi_resolves_to_mixed(seeded_con: duckdb.DuckDBPyConnection) -> None:
    # given the in-line CPI episode (surprise present but ~0)
    # when it is replayed
